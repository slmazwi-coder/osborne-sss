...
// ── Claude AI (Anthropic) ────────────────────────────────────────────────────
const SYSTEM_PROMPT = `You are a warm, knowledgeable and friendly assistant for Osborn Senior Secondary School in Mount Ayliff, Eastern Cape, South Africa.
...
School details:
- Name: Osborn Senior Secondary School (Osborn SSS)
- Location: Tshungwana A/A, Mount Frere, Eastern Cape
- District: Alfred Nzo West Education District
- Municipality: Umzimvubu Local Municipality
- Phone: 083 210 5055
- Email: osbornsss@email.com
- Website: osbornhighschool.com
- Facebook: Osborn Senior Secondary School
- Motto: "Let There Be Light"
- Principal: Mr N Siyangaphi
- School hours: Monday–Friday 07:30–15:30
- Grades: Grade 8 to Grade 12
- Type: Public No-Fee School
- Learners: Approximately 1,743
- Teachers: 54
...
School colours: Dark Forest Green (#1B4D2E), Cream, Gold/Yellow (#C9A84C)
...
async function askClaude(userMessage: string): Promise<string> {
  try {
    ...
    return 'I\'m having trouble connecting right now. Please contact the school directly at 039 940 4284 or osbornsss@email.com.';
  }
}
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: 'bot',
      createdAt: Date.now(),
      text: "👋 Hello! Let me help you with anything about Osborn Senior Secondary School! Whether it's admissions, boarding, fees, results, activities or anything else — just ask and I'll be happy to assist.",
    },
  ]);
  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-[#C9A84C] text-[#1B4D2E] shrink-0">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <div className="font-bold text-sm leading-tight truncate">Osborn Assistant</div>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 bg-gray-50 scroll-smooth">
            {messages.map((m) => (
              ...
              >
                {m.role === 'bot' && (
                  <div className="w-6 h-6 rounded-full bg-[#C9A84C] flex items-center justify-center shrink-0 mb-0.5 text-[#1B4D2E]">
                ...
                <div
                  className={`max-w-[80%] rounded-2xl px-3 py-2.5 text-sm leading-relaxed shadow-sm whitespace-pre-wrap ${
                    m.role === 'user'
                      ? 'bg-[#1B4D2E] text-white rounded-br-sm'
            ...
            {isTyping && (
              <div className="flex items-end gap-1.5">
                <div className="w-6 h-6 rounded-full bg-[#1B4D2E] flex items-center justify-center shrink-0">
                  <Sparkles size={11} className="text-white" />
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 justify-center">
                  {QUICK_QUESTIONS.map((q) => (
                    <button
                      key={q}
                      onClick={() => send(q)}
                      className="text-[11px] px-2.5 py-1 rounded-full bg-white border border-[#1B4D2E]/25 text-[#1B4D2E] hover:bg-[#143D24] hover:text-white transition-colors font-medium shadow-sm"
                    >
                      {q}
                    </button>
                  ))}
          ...
          <div className="p-3 bg-white border-t border-gray-100 shrink-0">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#1B4D2E]/20 focus:border-[#1B4D2E]/40 transition-all bg-gray-50 placeholder:text-gray-400"
              ...
              <button
                ...
                className="bg-[#1B4D2E] hover:bg-[#143D24]/90 text-white px-3 py-2 rounded-xl transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center shrink-0"
